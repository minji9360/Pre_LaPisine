...
int main_array(void)
{
	...
	// 값은 초기화를 반드시 해야 함. 하지 않으면 더미값이 들어감
	/*int arr[10];
	for (int i = 0; i < 10; i++)
	{
		printf("%d\n", arr[i]);
	...
	// 배열 크기는 항상 상수로 선언
	// size를 배열 크기에 넣을 수는 없음
	// 선언하지 않은 값부터는 자동으로 0으로 초기화
	// 애초에 선언하지 않으면 더미값이지만, 배열 10개 중 2개 선언하면 8개는 자동 0
	/*int size = 10;
	int arr[10] = { 1,2 };
	for (int i = 0; i < 10; i++)
	{
	...
	// 문자열 끝에는 "끝"을 의미하는 NULL 숫자 '\0'이 포함되어야 함
	...
	// 참고 2: 0~127 아스키코드 정수값에 해당하는 문자 확인
	for (int i = 0; i < 128; i++)
	{
		printf("아스키코드 정수 %d : %c\n", i, i);
	}
